...
const Job = ({ job }) => {
    const { companyName, address, logoImage, position, salary, workType, id } =
        job;
    return (
            ...
            <div className="my-4">
                ...
                <div className="my-2">
                    {workType.map((wt) => (
                        <button
                            key={id}
                            className="mr-2 border-2 border-[#A087FF] text-[#A087FF] hover:bg-[#A087FF] hover:text-white px-4 font-medium"
                        >
                            {wt}
                        </button>
                    ))}
